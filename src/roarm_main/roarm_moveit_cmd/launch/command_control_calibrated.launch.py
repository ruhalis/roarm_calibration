...
def generate_demo_launch(moveit_config, launch_package_path=None):
    """
    Launches a self contained demo with calibration parameters loaded

    Includes:
     * static_virtual_joint_tfs
     * robot_state_publisher
     * move_group (with calibration parameters)
     * moveit_rviz
    ...
    # Get path to calibration config
    roarm_moveit_cmd_share = get_package_share_directory('roarm_moveit_cmd')
    calibration_config_path = os.path.join(
        roarm_moveit_cmd_share,
        'config',
    ...
    # Declare calibration enable argument
    ld.add_action(
        DeclareLaunchArgument(
            "calibration_enabled",
            default_value="true",
            description="Enable/disable calibration offset compensation"
        )
    )
    ...
    # Launch move_group with calibration parameters
    ld.add_action(
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
            ...
            launch_arguments={
                'parameters': calibration_config_path
            }.items()
        )
    )
